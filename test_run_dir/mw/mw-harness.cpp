...
#endif
#include <iostream>
class mw_api_t: public sim_api_t<VerilatorDataWrapper*> {
    public:
    mw_api_t(Vmw* _dut) {
        dut = _dut;
        main_time = 0L;
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorIData(&(dut->io_din0)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_addr0)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_web0)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_csb0)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_clk0)));
        sim_data.outputs.push_back(new VerilatorIData(&(dut->io_dout0)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["mw.reset"] = 0;
    }
#if VM_TRACE
...
int main(int argc, char **argv, char **env) {
    ...
    Vmw* top = new Vmw;
    std::string vcdfile = "test_run_dir/mw/mw.vcd";
    ...
    Verilated::mkdir("test_run_dir/mw/logs");
    VerilatedCov::write("test_run_dir/mw/logs/coverage.dat");
#endif
    delete top;
    exit(0);
}
